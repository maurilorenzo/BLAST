...
```{r}
generate_data <- function(p=500, n_s=rep(500, 5), S=5, q_s=rep(5,5), k_0=10, seed=123, var='het', add_confounder = T){
  set.seed(seed)
  ...
  if(add_confounder){
    conf <- matrix(rnorm(p*2, 0, 0.3), ncol=2)
    Lambda_0[,1:2] <- Lambda_0[,1:2] + conf
  }
...
data_sim <- generate_data(p=p, n_s=n_s, S=S, q_s=q_s, k_0=k_0, seed=1, var=var, add_confounder = T)
...
for(s in 1:S){
  print(s)
  print(norm((tcrossprod(blast_est$Lambda_mean) + tcrossprod(blast_est$Gammas_mean[,,s]) - Gammas_0_outer[[s]] - Lambda_0_outer), type='F') / 
    norm(Gammas_0_outer[[s]]+ Lambda_0_outer, type='F'))
}
